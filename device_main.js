import { fetchData } from './device_api.js';
...
async function init() {
    ...
    for (let i in data.item) {
        ...
        if (data.item[i].icon != null) {
            ...
            marker.bindTooltip(`<b>${data.item[i].name}</b><br><b>Power: </b>${parseFloat(data.item[i].power).toLocaleString()} kW<br><b>Energy(Day): </b>${parseFloat(data.item[i].energy).toLocaleString()} kWh`, {
                offset: [0, -45],
                direction: 'top',
                permanent: true,
                direction: 'center',
